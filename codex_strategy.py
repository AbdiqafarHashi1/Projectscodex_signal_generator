...
# Timeframe auto-switch configuration
TF_AUTOSWITCH_ENABLED = env_flag("TF_AUTOSWITCH_ENABLED", True)
TF_LIST = [tf.strip() for tf in os.environ.get("TF_LIST", "1m,5m,15m").split(",") if tf.strip()]
TF_START = os.environ.get("TF_START", TF_LIST[0] if TF_LIST else INTERVAL)
TF_EVAL_LOOKBACK_TRADES = int(os.environ.get("TF_EVAL_LOOKBACK_TRADES", 10))
TF_MIN_TRADES = int(os.environ.get("TF_MIN_TRADES", 5))
TF_PNL_THRESHOLD = float(os.environ.get("TF_PNL_THRESHOLD", 0.0))
TF_SWITCH_COOLDOWN_CANDLES = int(os.environ.get("TF_SWITCH_COOLDOWN_CANDLES", 30))
TF_ALLOW_STEP_DOWN = env_flag("TF_ALLOW_STEP_DOWN", False)
...
_tf_sequence: List[str] = []
_current_tf_index = 0
_last_switch_candles = 0
...
CSV_DEFINITIONS = {
    'open_trades': {
        ...
        'columns': ['id','pair','signal','entry','sl','tp1','tp2','qty','initial_qty','remaining_qty','notional','est_fees','created_at','tp1_hit','status','sl_to_entry','realized_pnl','risk_frac','pattern','pattern_bias','rsi','score','atr_pct','volume_ratio','htf_confluence','atr_regime','interval']
    },
    'closed_trades': {
        'base': CLOSED_TRADES_CSV,
        'columns': ['id','pair','signal','entry','sl','tp1','tp2','qty','notional','est_fees','created_at','closed_at','result','close_price','gross_pnl','fees','net_pnl','balance_after','remaining_qty','partial','realized_total','risk_frac','pattern','pattern_bias','rsi','score','atr_pct','volume_ratio','htf_confluence','atr_regime','interval']
    },
    'diagnostics': {
...
def _initialize_timeframes():
    """Prepare the timeframe sequence and set the initial interval."""
    global _tf_sequence, _current_tf_index
    ...
    if not sequence:
        sequence = [fallback]
    elif fallback not in sequence:
        sequence.append(fallback)

    start_interval = fallback
    if TF_AUTOSWITCH_ENABLED:
        env_interval = os.environ.get("INTERVAL")
        candidate = env_interval or TF_START or (sequence[0] if sequence else fallback)
        if candidate:
            candidate = candidate.strip()
            if candidate:
                start_interval = candidate
                if start_interval not in sequence:
                    sequence.append(start_interval)

    _tf_sequence = sequence
    if start_interval not in _tf_sequence:
        _tf_sequence.append(start_interval)
    _current_tf_index = _tf_sequence.index(start_interval)
    _set_interval(start_interval, announce=False)


def _set_interval(new_interval: str, announce: bool = True) -> None:
    """Update global interval timing and notify."""
    global INTERVAL, INTERVAL_SECONDS, CYCLE_WAIT_SECONDS, _current_tf_index, _last_switch_candles, _tf_sequence

    if not new_interval:
        return

    normalized = new_interval.strip()
    if not normalized:
        return

    if normalized not in _tf_sequence:
        _tf_sequence.append(normalized)
    _current_tf_index = _tf_sequence.index(normalized)

    already = INTERVAL == normalized
    INTERVAL = normalized
    INTERVAL_SECONDS = interval_seconds(INTERVAL)
    CYCLE_WAIT_SECONDS = INTERVAL_SECONDS
    _last_switch_candles = 0
    ...
    if announce and not already:
        log_event(f"⏱️ Switching timeframe to {INTERVAL}")
        send_telegram(f"⏱️ Switching timeframe to {INTERVAL}")


def _pnl_for_interval(interval: str, lookback: int) -> Tuple[int, float]:
    if not interval or lookback <= 0:
        return 0, 0.0

    count = 0
    net_pnl = 0.0
    for trade in reversed(closed_trades):
        if trade.get('interval') != interval:
            continue
        try:
...
def maybe_switch_timeframe() -> None:
    """Evaluate recent performance and switch intervals when conditions are met."""
    global _last_switch_candles, _current_tf_index

    if not TF_AUTOSWITCH_ENABLED:
        return
    if not _tf_sequence or len(_tf_sequence) == 1:
        return

    try:
        _current_tf_index = _tf_sequence.index(INTERVAL)
    except ValueError:
        _tf_sequence.append(INTERVAL)
        _current_tf_index = len(_tf_sequence) - 1

    _last_switch_candles += 1
    cooldown = max(0, TF_SWITCH_COOLDOWN_CANDLES)
    if _last_switch_candles < cooldown:
        return

    current_interval = _tf_sequence[_current_tf_index]
    n_trades, net_pnl = _pnl_for_interval(current_interval, TF_EVAL_LOOKBACK_TRADES)
    if n_trades < TF_MIN_TRADES:
        return

    if net_pnl <= TF_PNL_THRESHOLD and _current_tf_index < len(_tf_sequence) - 1:
        next_interval = _tf_sequence[_current_tf_index + 1]
        _set_interval(next_interval)
        return

    if TF_ALLOW_STEP_DOWN and net_pnl > TF_PNL_THRESHOLD and _current_tf_index > 0:
        faster_interval = _tf_sequence[_current_tf_index - 1]
        _set_interval(faster_interval)


_initialize_timeframes()
...
# Live data helpers
# -----------------------
def get_klines_df(symbol: str, interval: Optional[str] = None, limit: int = 500) -> pd.DataFrame:
...
def open_simulated_trade(pair: str, signal: str, entry: float, sl: float, tp1: float, tp2: float, meta: Optional[Dict[str, Any]] = None):
    ...
    interval_tag = f"[{INTERVAL}]"
    send_telegram(
        f"{interval_tag} 📥 OPEN_SIM {pair} {signal} qty {qty:.6f} @ {entry:.2f} SL {sl:.2f} TP1 {tp1:.2f} TP2 {tp2:.2f}{pattern_note}" + stats_line
    )
    log_event(f"{interval_tag} Opened {pair} {signal} qty {qty:.6f} @ {entry:.2f}")
    return trade
...
def close_simulated_trade(trade_id: str, result: str, close_price: float):
    ...
    interval_tag = f"[{trade.get('interval', INTERVAL)}]"
    send_telegram(
        f"{interval_tag} 📤 CLOSE {trade['pair']} {signal} {result} @ {close_price:.2f} net {net:.2f} bal {account_balance:.2f}"
    )
    log_event(f"{interval_tag} Closed {trade['pair']} {signal} {result} @ {close_price:.2f} net {net:.2f}")
    return closed
...
def partial_close_trade(trade_id: str, fraction: float, close_price: float, label: str) -> Optional[Dict[str, Any]]:
    ...
    closed = {
        ...
        'interval': trade.get('interval', INTERVAL)
    }
    closed_trades.append(closed)
    append_named_csv('closed_trades', closed)
    interval_tag = f"[{trade.get('interval', INTERVAL)}]"
    send_telegram(
        f"{interval_tag} 📉 PARTIAL {trade['pair']} {signal} {label} qty {qty_to_close:.6f} @ {close_price:.2f} net {net:.2f} bal {account_balance:.2f}"
    )
    log_event(
        f"{interval_tag} Partial close {trade['pair']} {signal} {label} @ {close_price:.2f} qty {qty_to_close:.6f} net {net:.2f}"
    )
...
def diagnose_candle(pair: str, df: pd.DataFrame, ctx: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    try:
        ...
        diag_row = {
            ...
            'bias_1h': ctx['bias_1h'],
            'bias_4h': ctx['bias_4h'],
            'htf_slope_1h': ctx['htf_slopes'].get('1H'),
            'htf_slope_4h': ctx['htf_slopes'].get('4H'),
        ...
        if ctx['structure_bull']:
            close_score = ctx['long_score']
            threshold = LONG_SCORE_THRESHOLD
        elif ctx['structure_bear']:
            close_score = ctx['short_score']
    ...
    while True:
        try:
            ...
            for pair in PAIRS:
                ...
                candle_close = latest_ts + pd.Timedelta(seconds=INTERVAL_SECONDS)
